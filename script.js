...
class MinHeap {
	...
	heapify_down() {
		...
		while (current_index < this.size()) {
			...
			var rchild_index = (current_index * 2) + 2;
			if (lchild_index >= this.size() && rchild_index >= this.size()) {
				break;
			}
			else if (rchild_index >= this.size()) {
				let child_left = this.heap_array[lchild_index];
				if (current_element[0] < child_left[0]) {
					break;
				}
				else {
					this.heap_array[lchild_index] = current_element;
					this.heap_array[current_index] = child_left;
					current_index = lchild_index;
				}
			}
			else {
				var child_left = this.heap_array[lchild_index];
				var child_right = this.heap_array[rchild_index];
				if (current_element[0] < child_left[0] && current_element[0] < child_right[0]) {
					break;
				}
				else {
					if (child_left[0] < child_right[0]) {
						...
						this.heap_array[rchild_index] = current_element;
						this.heap_array[current_index] = child_right;
